import { brakes, ecus, exhausts, intakes, suspensions } from "./data/bolt_ons.js";
import { drivelines } from "./data/drivelines.js";
import { engines } from "./data/engines.js";
import { transmissions } from "./data/transmissions.js";
import { turbos } from "./data/turbos.js";
import type {
  BrakePart,
  ChassisFitment,
  Driveline,
...
function turboFit(engine: Engine | undefined, turbo: TurboPart): SwapMatch {
  if (turbo.subtype === "oem") {
    return { part: turbo, fit: "bolt-in", reasons: ["Заводской OEM наддув / впуск"] };
  }
  ...
  if (turbo.subtype === "itb") {
    if (engine?.aspiration === "na") {
      return {
        part: turbo,
        fit: "bolt-in",
        reasons: ["Многодроссельный впуск (ITB) оптимизирован для атмосферного мотора"],
      };
    }
    return {
      part: turbo,
      fit: "custom",
      reasons: ["Требуется переделка турбокомпрессорного тракта в безнаддувный"],
    };
  }
  ...
  if (turbo.subtype === "supercharger") {
    if (engine?.cylinders === 8) {
      return {
        part: turbo,
        fit: "bolt-in",
        reasons: ["Болт-он комплект шнекового нагнетателя в развал блока V8"],
      };
    }
    return {
      part: turbo,
      fit: "kit",
      reasons: ["Кастомные кронштейны крепления приводного ремня нагнетателя"],
    };
  }
  ...
  if (engine?.aspiration === "turbo" || engine?.aspiration === "twin-turbo") {
    return {
      part: turbo,
      fit: "bolt-in",
      reasons: ["Прямая замена штатной турбины на увеличенную улитку"],
    };
  }
  ...
  return {
    part: turbo,
    fit: "kit",
    reasons: ["Турбо-кит: требуется выпускной коллектор под турбину, слив масла и пайпинг"],
  };
}

function intakeFit(intake: IntakePart): SwapMatch {
  if (intake.subtype === "oem") {
    return { part: intake, fit: "bolt-in", reasons: ["Заводской OEM впускной тракт"] };
  }
  return {
    part: intake,
    fit: "bolt-in",
    reasons: ["Холодный впуск в штатные посадочные места"],
  };
}

function exhaustFit(exhaust: ExhaustPart): SwapMatch {
  if (exhaust.subtype === "oem") {
    return { part: exhaust, fit: "bolt-in", reasons: ["Заводская выхлопная система"] };
  }
  if (exhaust.subtype === "headers") {
    return {
      part: exhaust,
      fit: "kit",
      reasons: ["Равнодлинные коллекторы (требуется монтаж с подгонкой фланцев)"],
    };
  }
  return {
    part: exhaust,
    fit: "bolt-in",
    reasons: ["Прямоточная трасса по штатным точкам подвесов кузова"],
  };
}

function brakeFit(brake: BrakePart): SwapMatch {
  if (brake.subtype === "oem") {
    return { part: brake, fit: "bolt-in", reasons: ["Заводские OEM тормоза"] };
  }
  if (brake.subtype === "carbon-ceramic") {
    return {
      part: brake,
      fit: "kit",
      reasons: ["Углерод-керамика: требуются усиленные поворотные кулаки и адаптеры"],
    };
  }
  return {
    part: brake,
    fit: "bolt-in",
    reasons: ["Big Brake Kit со штатными переходными кронштейнами"],
  };
}

function suspensionFit(suspension: SuspensionPart): SwapMatch {
  if (suspension.subtype === "oem") {
    return { part: suspension, fit: "bolt-in", reasons: ["Заводская OEM подвеска"] };
  }
  if (suspension.subtype === "air-suspension") {
    return {
      part: suspension,
      fit: "kit",
      reasons: ["Пневмоподвеска: прокладка пневмомагистралей, ресивера и компрессора"],
    };
  }
  return {
    part: suspension,
    fit: "bolt-in",
    reasons: ["Винтовые стойки в штатные стаканы кузова"],
  };
}

function ecuFit(ecu: EcuPart): SwapMatch {
  if (ecu.subtype === "oem") {
    return { part: ecu, fit: "bolt-in", reasons: ["Заводская прошивка OEM"] };
  }
  if (ecu.subtype === "standalone") {
    return {
      part: ecu,
      fit: "kit",
      reasons: ["Спорт-ЭБУ: подключение через Plug-and-Play косу переходника"],
    };
  }
  return {
    part: ecu,
    fit: "bolt-in",
    reasons: ["Программный флеш ЭБУ через заводской OBD-II порт"],
  };
}
